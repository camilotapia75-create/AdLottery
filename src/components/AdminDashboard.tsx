...
export default function AdminDashboard({ session }: { session: AppSession }) {
  ...
  const [emailMessage, setEmailMessage] = useState(
    "Hi there!\n\nGreat news &mdash; Ad Lottery is officially live! Watch short video ads and get entered into daily cash prize draws.\n\nSign up now and start earning!"
  );
  ...
  const fetchWaitlist = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        setWaitlist({ list: data.waitlist, count: data.count });
      }
    } catch {}
  }, []);
  ...
  async function handleDraw() {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        const winnerNames = data.winners?.map((w: { name: string; email: string }) => w.name || w.email).join(", ");
        setDrawResult(`&#x2705; Draw complete! ${data.draw.winnersCount} winner(s): ${winnerNames}. Prize: $${data.prizePerWinner?.toFixed(4)} each.`);
        fetchStats();
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            { label: "Total Paid Out", value: `$${(stats?.totalEarningsPaid ?? 0).toFixed(4)}`, icon: "&#x1F4B8;" },
          ].map((stat) => (
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-white/60">{stats?.poolDrawn
              ? <span className="bg-green-500/20 text-green-300 px-3 py-1 rounded-full text-sm">&#x2705; Already drawn</span>
              : <span className="bg-yellow-500/20 text-yellow-300 px-3 py-1 rounded-full text-sm">&#x23F3; Pending draw</span>}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mb-6">
          ...
          <p className="text-white/60 text-sm mb-4">Select a date and run the lottery draw. Winners receive 70% of the pool, split equally.</p>
          <div className="flex gap-3 items-end">
            <div className="flex-1">
              <label className="text-sm text-white/70 mb-1 block">Draw Date</label>
              <input type="date" value={drawDate} onChange={(e) => setDrawDate(e.target.value)} className="w-full px-4 py-3 bg-white/20 text-white rounded-xl border border-white/20 focus:outline-none focus:ring-2 focus:ring-purple-400" />
            </div>
          ...
          {waitlist && waitlist.count > 0 ? (
            <>
              <div className="max-h-48 overflow-y-auto mb-4 space-y-1">
                ...
                {waitlist.count > 30 && (
                  <p className="text-white/40 text-xs text-center pt-1">+{waitlist.count - 30} more</p>
                )}
              </div>
              <div className="space-y-3 border-t border-white/10 pt-4">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-white/20 text-white rounded-xl border border-white/20 focus:outline-none focus:ring-2 focus:ring-purple-400 placeholder-white/40"
                />
                <textarea
                  ...
                  rows={4}
                  className="w-full px-4 py-3 bg-white/20 text-white rounded-xl border border-white/20 focus:outline-none focus:ring-2 focus:ring-purple-400 placeholder-white/40 resize-none"
                />
                <button
                  onClick={handleSendEmail}
                  disabled={sending}
                  className="px-6 py-3 bg-green-500 hover:bg-green-400 text-white font-bold rounded-xl transition-colors disabled:opacity-50"
                >
                  {sending ? "Sending..." : `&#x1F4E7; Send to All ${waitlist.count} Waitlisters`}
                </button>
                {sendResult && (
                  <div className={`p-3 rounded-xl text-sm ${sendResult.startsWith("&#x2705;") || sendResult.startsWith("✅") ? "bg-green-500/20 text-green-300" : "bg-red-500/20 text-red-300"}`}
                    dangerouslySetInnerHTML={{ __html: sendResult }} />
                )}
              </div>
            </>
          ) : (
          ...
          {loading ? <div className="text-white/40">Loading...</div> : !stats?.allDraws.length ? <div className="text-white/40 text-center py-8">No draws yet</div> : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-white">
                <thead><tr className="text-white/50 border-b border-white/10"><th className="text-left py-2 px-3">Date</th><th className="text-right py-2 px-3">Pool</th><th className="text-right py-2 px-3">Entries</th><th className="text-right py-2 px-3">Winners</th><th className="text-right py-2 px-3">Per Winner</th></tr></thead>
