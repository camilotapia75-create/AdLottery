      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        {referral?.referralCode && (
          <div className="bg-white rounded-2xl p-6 mb-6">
            ...
            <p className="text-gray-500 text-sm text-center mb-4">
              Share your link &mdash; when a friend signs up, you both get 5 free lottery entries!
            </p>
            <div className="flex gap-2 mb-3">
              <input
                readOnly
                value={`${window.location.origin}/register?ref=${referral.referralCode}`}
                className="flex-1 px-3 py-2 border border-gray-200 rounded-xl text-sm text-gray-500 bg-gray-50 truncate"
              />
              <button
                onClick={copyReferralLink}
                className="px-4 py-2 text-white text-sm font-semibold rounded-xl shrink-0 transition-colors"
            ...
            {referral.referralsCount > 0 && (
              <p className="text-center text-sm text-purple-600 font-medium">
                &#x2705; {referral.referralsCount} {referral.referralsCount === 1 ? "friend" : "friends"} referred
                &nbsp;&middot;&nbsp; {referral.bonusEntries} bonus entries earned
              </p>
            )}
          </div>
        )}
        ...
        {session.user.isAdmin && (
          <div className="text-center mb-4">
            <a href="/admin" className="text-white/80 underline text-sm hover:text-white">Admin Panel</a>
          </div>
        )}
