...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ totalUsers, totalViews, todayPool: todayPool?.totalPool ?? 0, watchersToday: todayPool?.viewCount ?? 0, poolDrawn: todayPool?.drawn ?? false, allDraws, totalEarningsPaid: totalEarningsPaid._sum.amount ?? 0 });
  } catch (error) {
