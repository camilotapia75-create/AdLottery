...
export async function GET() {
  ...
  try {
    const [pool, watchedToday, earningsResult, recentWins, recentDraws, totalEntries, totalWins] = await Promise.all([
    ...
    return NextResponse.json({ todayPool: pool?.totalPool ?? 0, watchersToday: pool?.viewCount ?? 0, watchedToday, poolDrawn: pool?.drawn ?? false, userEarnings: earningsResult._sum.amount ?? 0, totalEntries, totalWins, recentWins, recentDraws });
  } catch (error) {
